...
| 🔍 **五级检索** | Wiki 摘要 → 知识图谱 → **PPR 多跳扩散** → 向量混合检索（实体精确腿 + RRF 融合）→ **人物社群摘要**，多查询扩展 + 跨书共查 |
...
### 混合检索与召回率
纯向量检索对中文实体召回不足，为此引入「实体精确腿」：查询命中图谱节点名时，
用 `where_document` 限定分块必须包含实体名，再与纯向量结果做 RRF 融合。
黄金集（实体类问题，17 条）实测：

| 模式 | recall@1 | recall@3 | recall@5 |
|------|---------|---------|---------|
| 纯向量基线 | 29.4% | 58.8% | 64.7% |
| 混合检索 | **88.2%** | **88.2%** | **100%** |

复现：`python scripts/vector_recall_eval.py`（`--mode vector` 为基线）。
